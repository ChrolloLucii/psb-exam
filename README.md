...
Сначала был написан [конфиг](ansible.cfg) в котором указал основные параметры для работы ansible.
...
Далее написал [inventory.yaml](inventory.yml), где указал по какому адресу / порту подключаться, какого использовать пользователя и создал группу monitoring, в которую включен хост с которым мы будем работать.
Пропинговал хосты:
```bash
ansible all -m ping
```
...
P.S
#### В конце хочу выразить благодарность команде psbanka, экспертам, тьюторам и всем причастным. Хороший курс и команда, мне понрав💜
