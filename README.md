...
Далее написал [inventory.yaml](inventory.yaml), где указал по какому адресу / порту подключаться, какого использовать пользователя и создал группу monitoring, в которую включен хост с которым мы будем работать.
Пропинговал хосты:
```bash
ansible all -m ping
```
